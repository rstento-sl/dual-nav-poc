...
function ResizableSidenav() {
  ...
  var iconElement = $('<img src="' + initialIconSrc + '" alt="" role="presentation">');
...
$(document).ready(function() {
  ...
  var sectionIcons = {
    ...
    'Learn about the SnapLogic Platform': '<svg class="nav-section-icon" viewBox="0 0 16 16" fill="none" stroke="currentColor" stroke-width="1.4" stroke-linecap="round" stroke-linejoin="round"><path d="M2 3h5l1 1v9l-1-1H2V3z"/><path d="M14 3H9l-1 1v9l1-1h5V3z"/></svg>',
    ...
    'Cloud connectivity': '<svg class="nav-section-icon" viewBox="0 0 16 16" fill="none" stroke="currentColor" stroke-width="1.4" stroke-linecap="round" stroke-linejoin="round"><path d="M4 11a4 4 0 01-.7-7.9A5 5 0 0113 5a3 3 0 01-.3 6H4z"/></svg>',
    'Secrets Management': '<svg class="nav-section-icon" viewBox="0 0 16 16" fill="none" stroke="currentColor" stroke-width="1.4" stroke-linecap="round" stroke-linejoin="round"><rect x="4" y="7" width="8" height="7" rx="1"/><path d="M6 7V5a2 2 0 014 0v2"/><circle cx="8" cy="10.5" r="1" fill="currentColor"/></svg>',
    'SnapLabs': '<svg class="nav-section-icon" viewBox="0 0 16 16" fill="none" stroke="currentColor" stroke-width="1.4" stroke-linecap="round" stroke-linejoin="round"><path d="M6 2v4l-2 3v1h8v-1l-2-3V2"/><path d="M4 14h8"/></svg>',
    ...
    'Project Manager': '<svg class="nav-section-icon" viewBox="0 0 16 16" fill="none" stroke="currentColor" stroke-width="1.4" stroke-linecap="round" stroke-linejoin="round"><rect x="2" y="2" width="12" height="12" rx="1"/><path d="M2 6h12"/><path d="M6 6v8"/></svg>',
    'Public pattern library': '<svg class="nav-section-icon" viewBox="0 0 16 16" fill="none" stroke="currentColor" stroke-width="1.4" stroke-linecap="round" stroke-linejoin="round"><path d="M2 11l7-7 3 3-7 7H2v-3z"/></svg>',
  ...
  if (showFullTreeInit === 'true' && isProductView()) {
    // Browse by product collapsed in "All Products" view
  ...
  // -- on click --
  // Nav toggle arrows: expand/collapse without interfering with navigation
  // Nav expand/collapse: click link in a navparent to toggle
  $('nav.toc li.navparent > a').click( function(e) {
    var item = $(this).closest('li');
    if (item.hasClass('navexpand')) {
      e.preventDefault();
      e.stopPropagation();
      item.removeClass('navexpand').addClass('navcollapsed');
    } else {
      item.addClass('navexpand').removeClass('navcollapsed');
    }
  })

  // Span-based nav items (no link): click the text to expand/collapse
  $('nav.toc li > span:not(.nav-toggle)').click( function(e) {
    var item = $(this).closest('li');
...
$(window).on("resize", function () {
  ...
  function seedFromUrlParams() {
    ...
    if (pView === 'products' && !sessionStorage.getItem('activeNav')) {
      sessionStorage.setItem('activeNav', 'products');
      sessionStorage.setItem('showFullTree', 'true');
    }
    if (pView === 'tasks' && !sessionStorage.getItem('activeNav')) {
      sessionStorage.setItem('activeNav', 'tasks');
      sessionStorage.setItem('showFullTaskTree', 'true');
    }
    if (pCategory && !sessionStorage.getItem('filteredTaskSection')) {
      sessionStorage.setItem('filteredTaskSection', pCategory);
    }
    if (pProduct && !sessionStorage.getItem('filteredProduct')) {
      sessionStorage.setItem('filteredProduct', pProduct.toUpperCase());
    }
  }
  seedFromUrlParams();
  ...
  function updateUrlState() {
    ...
    var filteredSection = sessionStorage.getItem('filteredTaskSection');
    var filteredProduct = sessionStorage.getItem('filteredProduct');
    var showFullTree = sessionStorage.getItem('showFullTree');
    var showFullTaskTree = sessionStorage.getItem('showFullTaskTree');

    if (activeNav === 'products' || showFullTree === 'true') {
      params.set('view', 'products');
    } else if (activeNav === 'tasks' || showFullTaskTree === 'true') {
      params.set('view', 'tasks');
    }
    if (filteredSection) {
      params.set('category', filteredSection);
    }
    if (filteredProduct) {
      params.set('product', filteredProduct.toLowerCase());
    }
  ...
  if ($toc.length > 0) {
    ...
    const isOnAllTasksLanding = currentPath.includes('/tasks-about.html') || currentPath.match(/\/tasks\/index\.html/);
    ...
    $(document).on('click', 'nav.toc a:not(.nav-view-link)', function() {
      ...
      if (!isLandingPage) {
        ...
        // Clicking a landing page from the nav should filter to that category
        sessionStorage.removeItem('activeNav');
        sessionStorage.removeItem('showFullTree');
        sessionStorage.removeItem('showFullTaskTree');
      }
    ...
    function rebindNavHandlers() {
      $('nav.toc li.navparent > a').off('click').on('click', function(ev) {
        var item = $(this).closest('li');
        if (item.hasClass('navexpand')) {
          ev.preventDefault();
          ev.stopPropagation();
      ...
      $('nav.toc li > span:not(.nav-toggle)').off('click').on('click', function(ev) {
        var item = $(this).closest('li');
        if (item.hasClass('navparent')) {
          ev.stopPropagation();
          item.toggleClass('navexpand');
    ...
    // Handle "Browse by goal" click — navigate to All Tasks landing page
    $(document).on('click', '.nav-view-button-tasks', function(e) {
      ...
      var logoHref = $('a.header-logo-link').attr('href') || '../index.html';
      var basePrefix = logoHref.replace('index.html', '');
      window.location.href = basePrefix + 'tasks/goals/tasks-about.html';
    });
  ...
  if (isInProductViewForNav) {
    ...
    if (isTaskView) {
      ...
      if (!domFilterHandled && filteredTaskSection && showFullTaskTree !== 'true') {
        ...
        $categoryItems.each(function() {
          var $cat = $(this);
          var catName = $cat.children('span:not(.nav-toggle), a').first().text().trim();
          ...
          if (isCategoryMatch) {
            ...
            if (!filteredCategory || catName === filteredCategory) {
              ...
              $headings.each(function() {
                var $h = $(this);
                var hName = $h.children('span:not(.nav-toggle), a').first().text().trim();
